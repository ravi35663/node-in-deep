...
/*
    ...
    -> the event loop efficiently look out for the asynchronous tasks and move to other async task
       concurrently and when the task is done it keep looking other tasks into the event loop if any 
       comes after that.
...
/*
   List down the tasks which would be done asynchronously using the event loop?
   1) I/O Operations: 
