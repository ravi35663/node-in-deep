...
-> Node.js has one more important thing present which is known as Event-Loop which 
   will keep track of all the request/events present in the event queue.
-> Event-loop will check which kind of request user has send and if the request is a 
   asynchronous/non-blocking request then it sent response to the user without waiting 
   any other request to complete.

-> if the request is blocking request then that request went to the thread-pool which 
   have threads available to perform blocking operations.
   thread-pool: it is a collections of threads.Thread is also known as worker
-> once the job of blocking operation is completed by the thread, the thread come 
   back to the thread-pool and it will return the result to the request.
