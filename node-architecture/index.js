...
==> Node.js is combination  of two things:
    ...
    ==> V8-Engine:
      ...
      -> V8 engine help node.js to identify javascript in node.js and convert that javascript code into 
         C++ machine level code that can be understood by machine.
    ...
    ==> Event-Queue:
      -> the work of event queue is to keep the incoming requests of client and pass it into the event 
         loop.
    ...
    ====> Flow of Node.js Architecture:
      ...
      -> Event loop check if request is non-blocking, it execute it by itself(Event loop), otherwise the 
         request sent to the thread pool
      -> When thread pool receive the request from event loop, it execute them and result of the request 
         sent back to event loop.
*/
