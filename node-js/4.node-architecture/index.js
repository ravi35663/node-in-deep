...
==> Node.js is combination  of two things:
    ...
    ==> Event-Loop:
      ...
      -> If any blocking events/tasks comes to the event loop, event loop offload that task 
         and send it to the thread pool to execute this task and then event loop continue 
         with small tasks.
    ...
    ==> Thread pool:
      ...
      -> The event loop offload the heavy task and send it to the thread pool, thread pool 
         pick that task and assign a thread to execute them, once that execution of that 
         heavy/blocking task is done response send back to the event loop by thread pool.
    ...
    ====> Flow of Node.js Architecture:
      ...
      -> When thread pool receive the request from event loop, it execute them and result 
         of the request sent back to event loop.
*/
